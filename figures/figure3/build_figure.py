...
def generate_and_save_plot():
    fig, ax = plt.subplots(figsize=(8.5 / 2.54, 7.5 / 2.54), constrained_layout=True)
    ax.axis("off")
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ...
    for y, label in zip(seq_ys, seq_labels):
        ...
        ax.add_patch(
            Rectangle(
                (x, y), w, box_h, linewidth=1, edgecolor="black", facecolor="white"
            )
        )
        ax.text(
            x + w / 2, y + box_h / 2, label, ha="center", va="center", fontsize=fontsize
        )

    for y0, y1 in zip(seq_ys, seq_ys[1:]):
        ax.annotate(
            "",
            xy=(seq_x + cont_w / 2, y1 + box_h),
    ...
    par_labels = ["Fence Detection", "Fumarole Detection", "Degraded Classifier"]
    ...
    output_path = pathlib.Path.cwd() / "classification-workflow-overview.png"
