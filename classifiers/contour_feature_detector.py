"""
Detection of features based on whether the sum of contours in an region exceeds some
specified threshold.
...
def detect_feature(image, config):
    """
    Determine if a feature is visible in an image based on whether the sum of the
    contours exceeds some specified threshold.
    ...
    image = cv.adaptiveThreshold(
        image, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY_INV, 21, 2
    )
